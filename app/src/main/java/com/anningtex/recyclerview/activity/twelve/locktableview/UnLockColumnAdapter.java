...
public class UnLockColumnAdapter extends RecyclerView.Adapter<UnLockColumnAdapter.UnLockViewHolder> {
    ...
    private TableViewAdapter.OnItemChildSelectedListenter mItemChildSelectedListenter;
    ...
    public void setItemChildSelectedListenter(TableViewAdapter.OnItemChildSelectedListenter itemChildSelectedListenter) {
        mItemChildSelectedListenter = itemChildSelectedListenter;
    }
    ...
    private void createRowView(LinearLayout linearLayout, List<String> datas, boolean isFristRow, int mMaxHeight, final int position) {
        ...
        for (int i = 0; i < datas.size(); i++) {
            ...
            int lastIndexOf = datas.get(i).lastIndexOf("\n");
            if (lastIndexOf != -1) {
                SpannableStringBuilder style = new SpannableStringBuilder(datas.get(i));
                style.setSpan(new BackgroundColorSpan(Color.GRAY), lastIndexOf, datas.get(i).length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                style.setSpan(new ForegroundColorSpan(Color.WHITE), lastIndexOf, datas.get(i).length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            ...
            if (mOnItemClickListenter != null) {
                textView.setOnClickListener(v -> {
                    ...
                    if (mItemChildSelectedListenter != null) {
                        mItemChildSelectedListenter.onItemSelected(v, position, textView);
                    }
                    if (isLockFristRow) {
                        if (mOnItemChildClickListenter != null) {
                            mOnItemChildClickListenter.onItemClick(v, position, finalI);
                        }
                        if (mOnItemClickListenter != null) {
                            mOnItemClickListenter.onItemClick(v, position, finalI);
                        }
                    } else {
                        if (position != 0) {
                            if (mOnItemClickListenter != null) {
            ...
            if (mOnItemLongClickListenter != null) {
                textView.setOnLongClickListener(v -> {
                    ...
                    if (isLockFristRow) {
                        mOnItemLongClickListenter.onItemLongClick(v, position + 1, finalI);
                    } else {
                        if (position != 0) {
                            mOnItemLongClickListenter.onItemLongClick(v, position, finalI);
                        }
                    }
                    return true;
                });
