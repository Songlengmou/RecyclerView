...
public class LockColumnAdapter extends RecyclerView.Adapter<LockColumnAdapter.LockViewHolder> {
    ...
    public void onBindViewHolder(LockViewHolder holder, final int position) {
        ...
        if (!isLockFristRow) {
            if (position == 0) {
                holder.mTextView.setBackgroundColor(ContextCompat.getColor(mContext, mFristRowBackGroudColor));
        ...
        if (mOnItemLongClickListenter != null) {
            holder.mTextView.setOnLongClickListener(new View.OnLongClickListener() {
                ...
                public boolean onLongClick(View v) {
                    ...
                    if (isLockFristRow) {
                        // TODO
                        mOnItemLongClickListenter.onItemLongClick(v, position + 1, 0);
                    } else {
                        if (position != 0) {
                            // TODO
                            mOnItemLongClickListenter.onItemLongClick(v, position, 0);
                        }
                    }
                    return false;
                }
