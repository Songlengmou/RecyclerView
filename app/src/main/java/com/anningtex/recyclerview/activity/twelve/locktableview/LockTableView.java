...
import com.anningtex.recyclerview.activity.twelve.xrecyclerview.ProgressStyle;
import com.anningtex.recyclerview.activity.twelve.xrecyclerview.XRecyclerView;
...
public class LockTableView {
    ...
    private OnItemChildClickListenter mOnItemChildClickListenter;
    ...
    public void setOnItemChildClickListenter(OnItemChildClickListenter onItemChildClickListenter) {
    ...
    private void initAttrs() {
        ...
        maxRowHeight = 60;
        mNullableString = "N/A";
        mTableHeadTextColor = R.color.bei_jin;
...
//            Log.e("每行数据", mTableRowDatas.toString());
    ...
    private void initView() {
        mColumnTitleView = mTableView.findViewById(R.id.lockHeadView_Text);
        mLockHeadView = mTableView.findViewById(R.id.lockHeadView);
        mUnLockHeadView = mTableView.findViewById(R.id.unLockHeadView);
        mLockScrollView = mTableView.findViewById(R.id.lockHeadView_ScrollView);
        mUnLockScrollView = mTableView.findViewById(R.id.unlockHeadView_ScrollView);

        //表格主视图
        mTableScrollView = mTableView.findViewById(R.id.table_scrollView);
        ...
        mTableViewAdapter.setOnItemChildClickListenter(mOnItemChildClickListenter);

        mTableViewAdapter.setClick(new Click() {
            @Override
            public void onItemClick(int position, int index) {
                if (mClick != null) {
                    mClick.onItemClick(position + 1, index + 1);
                }
            }
        });

        mTableViewAdapter.setLongClick(new LongClick() {
            @Override
            public void onItemClick(int position, int index) {
                if (mLongClick != null) {
                    mLongClick.onItemClick(position, index);
                }
            }
        });
        ...
        if (mOnItemSeletor != 0) {
            ...
            mTableViewAdapter.setOnItemSeletor(R.color.dash_line_color);
        }
    ...
    private void creatHeadView() {
        if (isLockFristColumn && mColumnMaxWidths.size() > 0 && mRowMaxHeights.size() > 0) {
    ...
    public interface OnItemClickListenter {
         ...
         * @param position 点击位置
         */
        void onItemClick(View item, int position, int index);
    }

    public interface OnItemChildClickListenter {
    ...
    public interface OnItemLongClickListenter {
         ...
         * @param position 点击位置
         */
        void onItemLongClick(View item, int position, int index);
    }
}
